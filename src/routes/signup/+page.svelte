<script lang="ts">
  import { AlertCircle, ArrowLeft, Zap, CheckCircle2 } from '@lucide/svelte';
  ...
  async function handleSignup() {
    ...
    if (!data.session) {
      sessionStorage.removeItem('sekta-redirect');
      success = 'Pendaftaran berhasil! Cek email kamu untuk konfirmasi akun.';
    }
  }
...
<div class="min-h-full flex flex-col transition-colors duration-300 {isDark ? 'text-slate-100' : 'text-slate-900'}">
  ...
  <div class="px-6 pt-6 pb-8">
    ...
    <h1 class="text-[28px] font-black tracking-tight leading-tight mb-2">
      Gabung sekarang<br />dan mulai belajar! 🚀
    </h1>
  ...
  <div class="px-6 flex-1">
    <div class="space-y-4 mb-5">
      <div>
        <label for="signup-name" class="block text-xs font-bold mb-1.5 {isDark ? 'text-slate-400' : 'text-slate-600'}">Nama Lengkap</label>
        <input
          id="signup-name"
          type="text"
          bind:value={name}
      ...
      <div>
        <label for="signup-username" class="block text-xs font-bold mb-1.5 {isDark ? 'text-slate-400' : 'text-slate-600'}">Username</label>
        <input
          id="signup-username"
          type="text"
          bind:value={username}
      ...
      <div>
        <label for="signup-email" class="block text-xs font-bold mb-1.5 {isDark ? 'text-slate-400' : 'text-slate-600'}">Email</label>
        <input
          id="signup-email"
          type="email"
          bind:value={email}
      ...
      <div>
        <label for="signup-password" class="block text-xs font-bold mb-1.5 {isDark ? 'text-slate-400' : 'text-slate-600'}">Password</label>
        <input
          id="signup-password"
          type="password"
          bind:value={password}
      ...
      {#if success}
        <div class="border p-3 rounded-2xl flex items-center text-xs font-semibold {isDark ? 'bg-emerald-950/30 border-emerald-900/50 text-emerald-200' : 'bg-emerald-50 border-emerald-200 text-emerald-700'}">
          <CheckCircle2 class="w-4 h-4 mr-2 flex-shrink-0" /> {success}
        </div>
      {/if}
    </div>
    ...
    <button
      ...
      class="flex items-center justify-center w-full border rounded-2xl py-3.5 px-4 font-bold mb-6 transition-all active:scale-95 {isDark
